...
const Stepper = ({ currentStep }) => {
  const steps = [
    { num: 1, label: "Room Details" },
    { num: 2, label: "Design Instructions" },
    { num: 3, label: "Select Products" },
    { num: 4, label: "Room Generation" },
  ];

  return (
    <aside className="w-full md:w-64 lg:w-72 shrink-0">
      <div className="neomorph-raised rounded-[2rem] p-8 bg-background h-full min-h-[400px] flex flex-col relative">
        <h2 className="font-headline font-bold text-xl mb-8 text-on-surface">Steps</h2>
            ...
            return (
              <div key={step.num} className="flex items-center gap-4 relative z-10">
                ...
                {step.num < steps.length && (
                  <div className="absolute left-[15px] top-8 h-8 w-[2px] bg-primary/30 z-0"></div>
                )}
