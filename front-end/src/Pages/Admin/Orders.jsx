  ...
  const fetchOrdersList = async () => {
    try {
      ...
      const formatted = rawOrders.map((o) => ({
        ...
        _id: o._id || o.id,
        customerName: o.customer?.name || o.customerInfo?.name || o.shippingAddress?.fullName || (o.buyerId?.profile ? `${o.buyerId.profile.firstName} ${o.buyerId.profile.lastName || ''}`.trim() : (o.userId?.profile ? `${o.userId.profile.firstName} ${o.userId.profile.lastName || ''}`.trim() : "Customer")),
        customerEmail: o.customerInfo?.email || o.buyerId?.authentication?.email || o.userId?.authentication?.email || o.customerEmail || "",
        customerPhone: o.customer?.phone || o.customerInfo?.phone || o.shippingAddress?.phone || o.customerPhone || "",
        sellerName: o.sellerId?.sellerProfile?.businessName || (o.sellerId?.profile ? `${o.sellerId.profile.firstName} ${o.sellerId.profile.lastName || ''}`.trim() : o.sellerName || "Store Seller"),
        itemsCount: o.itemsCount || (o.items ? o.items.length : 1),
        totalAmount: `EGP ${(o.totalAmount || o.grossTotalAmount || 0).toLocaleString()}`,
        status: o.status || "PENDING",
        date: o.createdAt ? new Date(o.createdAt).toISOString().split("T")[0] : new Date().toISOString().split("T")[0],
        shippingAddress: typeof o.shippingAddress === "string" ? o.shippingAddress : (o.customer?.address?.street ? `${o.customer.address.street}, ${o.customer.address.city}` : o.shippingAddress?.street ? `${o.shippingAddress.street}, ${o.shippingAddress.city}` : "Cairo, Egypt"),
        paymentMethod: o.paymentMethod || "Card / Cash on Delivery",
        items: (o.items || []).map((item) => ({
          name: item.name || item.productTitle || "Furniture Item",
          qty: item.quantity || item.qty || 1,
          price: `EGP ${(item.price || 0).toLocaleString()}`,
        })),
        timeline: o.timeline || [
