...
const Cart = () => {
  ...
  const handleCheckout = async (e) => {
    ...
    if (!shippingAddress.phone || !shippingAddress.city || !shippingAddress.street) {
      setError("Please fill in all required shipping address fields (Phone, City, Street).");
      return;
    }
  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="font-headline font-bold text-2xl text-on-surface mb-2">Your Cart is Empty</h2>
        <p className="text-on-surface-variant max-w-md mb-8">
          You haven't added any products to your cart yet. Explore your AI room generations and select furniture pieces to place an order.
        </p>
        <Link
          to="/projects"
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-on-surface-variant text-sm mt-1">
            Review your selected furniture pieces grouped by seller and complete your order.
          </p>
        </div>
          ...
          {groupedBySeller.map((group, gIdx) => (
            <div key={group.sellerId} className="bg-background rounded-3xl p-6 neomorph-raised border border-outline-variant/30">
              ...
              <div className="mt-4 pt-3 border-t border-outline-variant/20 flex justify-between items-center text-xs font-semibold text-on-surface-variant">
                <span>Batch Subtotal ({group.items.reduce((s, i) => s + i.quantity, 0)} items)</span>
                <span className="font-bold text-sm text-primary">{group.subtotal} EGP</span>
              </div>
            </div>
          ))}
        </div>
          ...
          <form onSubmit={handleCheckout} className="bg-background rounded-3xl p-6 neomorph-raised border border-outline-variant/30 flex flex-col gap-6">
            ...
            <div className="pt-4 border-t border-outline-variant/20">
              ...
              <div className="p-4 rounded-2xl flex items-center justify-between border border-primary/40 bg-primary/10 text-primary shadow-md">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-bold text-sm text-on-surface block">Stripe Card & Online Payment</span>
                    <span className="text-xs text-on-surface-variant">256-bit SSL encrypted & secure checkout</span>
                  </div>
                </div>
            ...
            <div className="neomorph-inset p-4 rounded-2xl flex flex-col gap-2 mt-2">
              <div className="flex justify-between text-xs text-on-surface-variant">
                <span>Total Batches (Sellers)</span>
                <span className="font-bold text-on-surface">{groupedBySeller.length} Order(s)</span>
              </div>
              <div className="flex justify-between text-xs text-on-surface-variant">
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>
                    {paymentMethod === "stripe" ? "Proceed to Stripe Payment" : "Place Orders (Cash on Delivery)"}
                  </span>
                </>
              )}
            </button>
