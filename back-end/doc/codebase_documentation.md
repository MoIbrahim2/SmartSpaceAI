...
The Apartment model matches the following structured BSON schema representation:

*   **`ownerId`** (ObjectId, required): Reference to the owner's `users._id`.
*   **`name`** (String, required): The name of the apartment.
*   **`description`** (String): Optional description of the apartment.
*   **`coverImage`**: Object containing the cover image upload details:
    *   `url` (String): Relative path/URL to the uploaded cover image.
    *   `storageProvider` (String): Storage provider type (defaults to `'local'`).
    *   `fileName` (String): Unique filename generated by Multer.
    *   `uploadedAt` (Date): Date of upload.
*   **`location`**: Object containing the physical address properties:
...
*   **`status`** (String, enum: `['ACTIVE', 'ARCHIVED']`, required): Active listing status (defaults to `'ACTIVE'`).
*   **`createdAt`** (Date, automatic): Timestamp when the apartment document was created.
*   **`updatedAt`** (Date, automatic): Timestamp when the apartment document was last updated.
